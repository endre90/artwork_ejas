...
def plot_schedule_grid(int_mat, label_mat, date_labels, operators):
    ...
    # --- Build the Status Colormap ---
    # 0: Green (Operations)
    # 1: Grey (E - Empty/Ergo)
    # 2: Pastel Pink (S - Sick)
    # 3: Pastel Blue (L - Leave)
    # 4: Pastel Purple (T - Training)
    # 5: White (N - No Production, will be hatched)
    colors = ["#a1d99b", "#f5f5f5", "#fbb4ae", "#9e9ac8", "#fdbb84", "#ffffff"] 
    ...
    legend_elements = [
        ...
        Patch(facecolor='#f5f5f5', edgecolor='white', label='E (Absent)'),
        Patch(facecolor='#fbb4ae', edgecolor='white', label='S (Supervision)'),
    ...
    ax.legend(handles=legend_elements, loc='lower center', bbox_to_anchor=(0.5, 1.02), ncol=6, frameon=False, fontsize=10)
